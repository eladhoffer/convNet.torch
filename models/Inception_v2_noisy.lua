...
local function SBN(nInputDims, eps, ...) 
    eps = eps or 1e-3
    local m = nn.Sequential()
    m:add(nn.SpatialBatchNormalization(nInputDims, eps, ...))
...
local SpatialBatchNormalization = SBN
---------------------------------------Inception Modules-------------------------------------------------
local Inception = function(nInput, n1x1, n3x3r, n3x3, dn3x3r, dn3x3, nPoolProj, type_pool,stride)
    ...
    if n3x3>0 and n3x3r>0 then
        ...
        if BNInception then
            Module_3x3:add(SpatialBatchNormalization(n3x3r))
        end
    ...
    if dn3x3>0 and dn3x3r>0 then
        ...
        if BNInception then
            Module_d3x3:add(SpatialBatchNormalization(dn3x3r))
        end
...
part1:add(SpatialBatchNormalization(64))
part1:add(ReLU(true))
...
part1:add(SpatialBatchNormalization(192))
part1:add(ReLU(true))
...
part3:add(SpatialBatchNormalization(576))
part3:add(ReLU(true))
part3:add(Inception(576,0,128,192,192,256,0,'max',2))  --(4e) 576x14x14 -> 1024x7x7
part3:add(SpatialBatchNormalization(1024))
part3:add(ReLU(true))
